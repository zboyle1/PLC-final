...
public class Lex {
   ...
   private ArrayList<Token> tokenListInit(String inp) {
      ...
      while(i < inp.length()) {
         ...
         // Find a single character token
         } else if(tokenMap.containsKey(inp.charAt(i))) {
            list.add(new Token(String.valueOf(inp.charAt(i)),tokenMap.get(inp.charAt(i))));
            break; 
         } else if(lexeme == null) {
            /* Find valid tokens
            ...
            lexeme = String.valueOf(inp.charAt(i));
            i++;
            while(inp.charAt(i) != ' ' || tokenMap.containsKey(inp.charAt(i+1))) {
          ...
          * RegEx:([a-z]_?){6,8}
          */
         } else if(lexeme.matches("([a-z]_?){6,8}")) {
         ...
         * regex: [0-9]+
         */
         } else if(String.valueOf(inp.charAt(i)).matches("[0-9]?")) {
          ...
          * regex: [0-9]+\.[0-9]+
          */
         } else if(String.valueOf(inp.charAt(i)).matches("[0-9]+\\.[0-9]+")) {
          ...
          * regex: yes|no
          */
         } else if(String.valueOf(inp.charAt(i)).matches("yes|no")) {
   ...
   private HashMap<String, Integer> tokenMapInit() {
      ...
      map.put("<<<=",41);       // less than or equal to
      map.put(">>",42);         // greater than
      map.put(">>>=",43);       // greater than or equal to
      map.put("===",44);        // equal to
      map.put("!!!=",45);       // not equal to
      map.put("!!!",46);        // not
      map.put("&&&",47);        // and
      map.put("|||",48);        // or
